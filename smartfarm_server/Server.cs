...
namespace smartfarm_Server
{
    public class Server
    {
        ...
        public async Task StartAsync()
        {
            ...
            _listener.Start();
            Console.WriteLine($"[SERVER] Tcp 서버 가동  {_tcpPort}");
            ...
            while (true)
            {
                ...
                _ = HandleClientAsync(client);  // 클라 하나씩 따로 처리 (필요하면 나중에 수신도)
            }
        }
        ...
        private void SerialDataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            try
            {
                string line = _serialPort.ReadLine();     // 한 줄 읽기 (예: "25.3")
                line = line.Trim();                       // 공백/개행 제거

                Console.WriteLine($"[SERVER] 아두이노 : {line}");

                // 여기서는 그냥 아두이노 문자열을 그대로 클라들에게 전송
        ...
        // ===== WPF 클라이언트 개별 처리 (지금은 수신 안 쓰지만 껍데기만 만들어 둠) =====
